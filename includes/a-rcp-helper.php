 ...
 */
function adl_rec_post_first_image_or_default() {
    global $post;
    $first_img = '';
    ob_start();
    ob_end_clean();
    ...
    //if(empty($first_img)) {
    //    $first_img =  A_RCP_PLUGIN_URI .'/img/featured_image_placeholder.jpg';
    //}
    return $first_img;
}
 ...
 */
function adl_rec_post_check_min_wp_version($required_version) {
 ...
 *
 */
function adl_rec_post_warn_if_unsupported_wp() {
        $wp_ver = ! empty( $GLOBALS['wp_version'] ) ? $GLOBALS['wp_version'] : '(undefined)';
?>
        <div class="error notice is-dismissible"><p>
            <?php
                echo 'ADL Post Slider requires WordPress version ' .A_RCP_MINIMUM_WP_VERSION. ' or newer. It appears that you are running '.esc_html( $wp_ver ).' The plugin has been auto deactivated. Please <a href="https://wordpress.org/download/" target="_blank" title="download latest version of WordPress">upgrade</a> your WordPress Version';
                ?>
        </p></div>
<?php
